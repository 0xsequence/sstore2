...
contract TestSSTORE2Mapping {
  ...
  function addressOf1(bytes32 _key) external view returns (address) {
    return Create3.addressOf(SSTORE2Mapping.internalKey(_key));
  }

  function addressOf2(string calldata _key) external view returns (address) {
    return Create3.addressOf(SSTORE2Mapping.internalKey(keccak256(bytes(_key))));
  }
}
